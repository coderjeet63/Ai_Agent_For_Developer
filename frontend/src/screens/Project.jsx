...
const Project = () => {
  ...
  function WriteAiMessage(msg) {
    const messageObject = safeParse(msg)
    return (
  ...
  useEffect(() => {
    ...
    receiveMessage('project-message', data => {
      if (data.sender._id === 'ai') {
        const message = safeParse(data.message)
            ...
            <div className="actions flex gap-2">
              <button
                onClick={async () => {
                  ...
                  tempRunProcess.output.pipeTo(new WritableStream({ write: console.log }))
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-4 rounded-md w-96 max-w-full relative">
            ...
            <div className="users-list flex flex-col gap-2 mb-16 max-h-96 overflow-auto">
              {users.map(user => (
                <div key={user._id} className={`user cursor-pointer hover:bg-slate-200 ${Array.from(selectedUserId).includes(user._id) ? 'bg-slate-200' : ""} p-2 flex gap-2 items-center`} onClick={() => handleUserClick(user._id)}>
